...
왜 쓰는지: 이것은 n과의 거리 차이를 계산해서 1차 정렬 기준으로 사용하려는 거야. (n과 a의 거리) - (n과 b의 거리) 값을 반환하는 거지.
어떻게 구현하는지:
Math.abs(n - a): n과 a 사이의 **절댓값 차이(거리)**를 구해. (예: n=15, a=10 이면 abs(15-10)=5)
Math.abs(n - b): n과 b 사이의 **절댓값 차이(거리)**를 구해. (예: n=15, b=20 이면 abs(15-20)=5)
diffA - diffB와 같은 이 연산의 결과는 sort() 메소드에 정렬 순서를 결정하는 숫자를 반환해줘.
만약 a가 n에 더 가까우면 (diffA < diffB), 결과는 음수(-1)가 나와. (예: 5 - 10 = -5) -> a를 b보다 앞에 둬.
만약 b가 n에 더 가까우면 (diffB < diffA), 결과는 양수(1)가 나와. (예: 10 - 5 = 5) -> b를 a보다 앞에 둬.
만약 거리가 같으면 (diffA == diffB), 결과는 '0'이 나와. 이때 다음 기준으로 넘어갈 필요가 생겨!
|| a - b:
