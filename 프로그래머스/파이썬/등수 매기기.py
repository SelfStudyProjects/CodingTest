# 이중 for 반복문 로직으로 풀어보자
# 첫번째 반복문에서 2개 요소들이 있는 각 리스트에 접근, 이때 인덱스 값 설정(0부터 시작해서, 
# 1씩 늘어나게끔 하기), 두번째 반복문에서는 해당 리스트의 2개 요소들의 평균을 구하는 작업 시행 이때
# np.mean() 메소드를 활용해보면 좋을 듯, 값들을 대상으로 
# rank() 메소드를 활용해서 순위로 치환하는 방법이 좋을 듯?
