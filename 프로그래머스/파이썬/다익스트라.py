# 다시 보기
# 진짜 이해 안되는 파트 그러니 더더욱 계속 보기 특히!
# 사용된 알고리즘 및 자료구조는 아래와 같습니다.

'''
- **다익스트라 알고리즘**: 그래프에서 한 지점(출발 정점)에서 모든 노드까지의 최단 경로를 탐색하는 알고리즘. 가중치가 양수일 때 사용.
- **그래프(Adjacency List)**: 각 노드의 이웃 노드와 간선 가중치를 리스트/딕셔너리로 저장하는 구조.
- **우선순위 큐(Min-Heap)**: 최단 거리가 가장 작은 노드를 빠르게 선택하는 자료구조.
- **거리 테이블(Dictionary or List)**: 출발 노드에서 각 노드까지의 최단 경로 값을 저장 및 갱신하는 배열 또는 딕셔너리.
- **방문 여부(암묵적 체크)**: 이미 더 짧은 경로로 방문된 노드는 패스하여 불필요한 중복 연산을 방지함(continue 사용).
'''
