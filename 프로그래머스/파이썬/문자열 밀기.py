# A와 B를 문자열을 배열로 변환시켜주기, array() 함수 활용
# A를 기준으로 len()해서 전체 요소 개수, n으로 두기
# n을 기준으로 for 반복문 시행
# n은 for 반복문 시행되기 전에 0으로 설정
# if문 두기, A와 B 같은지 먼저 비교 후 같으면 result 0, 아니면 for 반복문으로 가는 로직
# for 반복문 시행 동안 아래 내용들 수행
# - pop()을 활용해서 맨 뒤 요소 제거
# - insert()를 활용해서 새로운 요소 추가하기
# - 시행 횟수 n += 1 하기
# - 위 과정들을 수행 후 B와 비교, 같으면 break 해서 retrun n 하기
# 만일 for 반복문 다 수행했는데 동일한 값이 없으면 result -1 하기

def solution(A, B):
    # A와 B가 이미 같은지 먼저 확인 (밀지 않아도 같은 경우: 0번 이동)
    if A == B:
        return 0

    # A를 문자열에서 리스트로 변환 (pop, insert를 활용하기 위함)
    # 파이썬에서 문자열은 불변(immutable)이기 때문에 변경 작업을 위해 리스트로 만듭니다.
    list_A = list(A) 
    
    # A의 길이 (즉, 몇 번까지 밀어볼 수 있는지 최대 횟수)
    # len(A)번 밀면 다시 원래 A의 상태로 돌아오게 됩니다.
    num_shifts = len(A) 
    
    # 문자열을 민 횟수를 저장할 변수
    # 0번 민 경우는 이미 위에서 처리했으므로 1부터 시작합니다.
    shift_count = 0 

    # num_shifts (A의 길이)만큼 반복하며 한 칸씩 밀어봅니다.
    # A의 길이와 B의 길이가 같다는 제한사항이 있으므로,
    # A의 길이 = B의 길이 = num_shifts 입니다.
    for _ in range(num_shifts): # for 루프 변수가 필요 없을 때는 _ (언더스코어) 사용
        shift_count += 1 # 한 칸 밀었으니 횟수 증가

        # - pop()을 활용해서 맨 뒤 요소 제거
        last_char = list_A.pop() 
        
        # - insert()를 활용해서 새로운 요소 (제거된 마지막 요소)를 맨 앞에 추가
        list_A.insert(0, last_char) 
        
        # 위 과정들을 수행 후, 현재 list_A가 B와 같은지 비교
        # 리스트를 문자열로 다시 합쳐서 비교해야 합니다.
        if "".join(list_A) == B:
            return shift_count # 같으면 현재까지 민 횟수 반환

    # for 반복문 다 수행했는데 동일한 값이 없으면 (즉, 위에서 return 되지 않으면)
    return -1 # -1 반환